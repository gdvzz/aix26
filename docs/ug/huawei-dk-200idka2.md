...
1. **设置拨码开关（可选）** 
    请确保开发者套件的拨码开关2、3、4（开关1为预留开关，当前无功能）的开关值为：2-OFF，3-ON，4-OFF。
    
    如果不是则要调整，否则将无法从SD读取镜像启动开发者套件。（注：外借的开发板已在正确位置。可跳过该步骤。）
...
1. 设置PC（本地电脑）的IP 为 `192.168.137.222`，以及子网掩码或子网掩码长度
    如何设置请参考：[以太网口远程登录（SSH方式）（Windows系统）↗]。
    如需要设置 **网关**（否则无法保存），则填写 `192.168.137.1`。 
2. 在电脑上启动命令行终端程序（比如 Windows 操作系统的 cmd，或者 powershell），并在命令行终端上执行 `ping 192.168.137.100`。如能看到如下信息，则表明电脑和开发板之间的网络是连通的。
    ```bash
